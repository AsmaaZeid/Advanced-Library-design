...
const GameCard = ({ game, darkMode }) => {
  ...
  return (
    <div onClick={() => navigate(`/games/${game.title}`)}
      ...
      style={{ boxShadow: dm ? '0 8px 32px rgba(109,40,217,.25)' : '0 8px 32px rgba(109,40,217,.12)' }}>
...
const AllGames = ({ darkMode }) => {
  ...
  useEffect(() => {
    setVisibleGames(games.filter(g => g.title.toLowerCase().includes(searchQuery.toLowerCase())));
  }, [searchQuery]);

  return (
    <div className={`min-h-screen w-full transition-colors duration-500 overflow-x-hidden ${bg}`}>
      {/* Soft bg glow */}
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div className={`absolute -top-40 left-1/4 w-96 h-96 rounded-full blur-[90px] ${dm?'bg-[#8B5CF6]/10':'bg-[#8B5CF6]/06'}`}/>
        <div className={`absolute bottom-0 right-1/4 w-80 h-80 rounded-full blur-[80px] ${dm?'bg-[#EC4899]/07':'bg-[#EC4899]/04'}`}/>
      </div>
      ...
      <div className="relative pt-16 h-[52vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute inset-0">
          <div className={`absolute top-[-15%] left-[-10%] w-[45%] h-[70%] rounded-full blur-[100px] ${dm?'bg-[#6D28D9]/20':'bg-[#8B5CF6]/12'} animate-pulse`}/>
          <div className={`absolute bottom-[-15%] right-[-10%] w-[45%] h-[70%] rounded-full blur-[100px] ${dm?'bg-[#EC4899]/15':'bg-[#EC4899]/08'} animate-pulse`} style={{ animationDelay:'700ms' }}/>
        </div>
        {/* Stars */}
        {[...Array(18)].map((_,i)=>(
        ...
        <div className="relative z-10 text-center px-6">
          <div className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full border mb-6 text-xs font-bold tracking-widest uppercase backdrop-blur-sm ${dm?'border-[#A78BFA]/30 bg-[#8B5CF6]/10 text-[#C4B5FD]':'border-[#A78BFA]/40 bg-[#8B5CF6]/08 text-[#6D28D9]'}`}>
          ...
          <p className={`max-w-2xl mx-auto text-lg leading-relaxed ${dm?'text-[#C4B5FD]':'text-[#6D28D9]'}`} style={{ fontFamily:"'Lora',serif" }}>
      ...
      <div className="max-w-2xl mx-auto px-6 -mt-8 mb-16 relative z-20">
        <div className={`relative flex items-center p-2 rounded-3xl border backdrop-blur-2xl shadow-2xl transition-all focus-within:ring-2 focus-within:ring-[#8B5CF6]/40 ${dm?'bg-[#1E1540]/90 border-[#2A1F55]':'bg-white/90 border-[#E9DEFF]'}`}
          style={{ boxShadow: dm?'0 8px 40px rgba(109,40,217,.2)':'0 8px 40px rgba(109,40,217,.1)' }}>
          ...
          <input type="text" value={searchQuery} onChange={e=>setSearchQuery(e.target.value)}
            ...
            className={`w-full px-4 py-3.5 bg-transparent outline-none text-base font-medium placeholder-[#A78BFA]/60 ${dm?'text-[#F3EEF8]':'text-[#2E1065]'}`}/>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 pb-20">
        <div className="flex items-center justify-between mb-8">
          <div>
            <p className={`text-xs uppercase tracking-[.3em] font-bold mb-1 ${dm?'text-[#A78BFA]':'text-[#8B5CF6]'}`}>✦ Available Roadmaps</p>
            <h2 style={{ fontFamily:"'Playfair Display',serif" }} className="text-2xl font-black">{visibleGames.length} Paths</h2>
          </div>
        </div>

        {visibleGames.length===0 ? (
          <div className={`py-20 text-center border-2 border-dashed rounded-3xl ${dm?'border-[#2A1F55] text-[#A78BFA]':'border-[#E9DEFF] text-[#8B5CF6]'}`}>
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {visibleGames.map((game,i)=>(
              <div key={i} className="animate-fade-in" style={{ animationDelay:`${i*80}ms` }}>
                <GameCard game={game} darkMode={dm}/>
              </div>
            ))}
          </div>
        )}
