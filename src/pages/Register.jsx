...
function Register({ darkMode }) {
  ...
  return (
    <div className={`min-h-screen flex items-center justify-center px-6 py-12 relative overflow-hidden ${darkMode?'bg-[#0D0618] text-[#F3EEF8]':'bg-[#FDF8FF]'}`}>
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div className="absolute -top-32 -right-32 w-96 h-96 rounded-full blur-[80px] bg-[#7C3AED]/15"/>
        <div className="absolute -bottom-32 -left-32 w-96 h-96 rounded-full blur-[80px] bg-[#EC4899]/15"/>
        {[...Array(20)].map((_,i)=>(<div key={i} className="absolute rounded-full" style={{ left:`${(i*17+5)%100}%`, top:`${(i*11+8)%100}%`, width:'2px', height:'2px', background:i%2===0?'#A78BFA':'#F472B6', animation:`star-twinkle ${1.5+i%4*0.5}s ${i*0.2}s ease-in-out infinite`, opacity:0.5 }}/>))}
      </div>
      ...
      <div className={`relative w-full max-w-xl p-10 rounded-[2.5rem] border shadow-2xl animate-zoom-in ${darkMode?'bg-[#140D24] border-[#2D1B5E]':'bg-white border-[#E9D5FF]'}`}
        ...
        <p className={`mt-6 text-center text-sm ${darkMode?'text-[#C4B5FD]':'text-[#6B21A8]'}`}>
          Already have an account?{" "}<Link to="/login" className={`font-black hover:underline ${darkMode?'text-[#F472B6]':'text-[#EC4899]'}`}>Login ✧</Link>
        </p>
      </div>
    </div>
  );
