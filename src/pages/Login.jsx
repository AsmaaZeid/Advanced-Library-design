...
function Login({ darkMode }) {
  ...
  const inputClass = `w-full px-5 py-4 rounded-2xl outline-none border transition-all text-sm focus:ring-2 ${darkMode?'bg-[#0D0618] border-[#2D1B5E] text-[#F3EEF8] focus:border-[#7C3AED] focus:ring-[#7C3AED]/20 placeholder-[#4C1D95]':'bg-[#F3EEF8] border-[#E9D5FF] focus:border-[#7C3AED] focus:ring-[#7C3AED]/10 placeholder-[#9333EA]'}`;
  const labelClass = `block text-[10px] uppercase tracking-widest font-bold mb-1.5 ${darkMode?'text-[#A78BFA]':'text-[#9333EA]'}`;
      ...
      <div className={`relative w-full max-w-md p-10 rounded-[2.5rem] border shadow-2xl animate-zoom-in ${darkMode?'bg-[#140D24] border-[#2D1B5E]':'bg-white border-[#E9D5FF]'}`}
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            ...
            <input type="email" name="email" placeholder="name@example.com" value={formData.email} onChange={handleChange} required className={inputClass}/>
          </div>
          <div>
