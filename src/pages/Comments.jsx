...
const CommentCard = ({ comment, currentUser, darkMode, onDelete, onDeleteReply, onReply }) => {
  ...
  const cardCls = darkMode ? 'bg-[#1E1540] border-[#2A1F55]' : 'bg-white border-[#E9DEFF]';
  const subtleText = darkMode ? 'text-[#A78BFA]' : 'text-[#8B5CF6]';
  ...
  return (
    <div className="group">
      <div className={`p-8 rounded-[2.5rem] border transition-all hover:shadow-xl ${cardCls}`}
        style={{boxShadow:darkMode?'0 0 30px rgba(109,40,217,.06)':'0 4px 20px rgba(109,40,217,.05)'}}>
        ...
        <button onClick={() => setIsReplying(!isReplying)}
          className={`text-xs font-black uppercase tracking-widest opacity-50 hover:opacity-100 transition-all ${isReplying ? 'text-red-400' : subtleText}`}>
        ...
        {isReplying && (
          <div className="mt-6 flex gap-3">
            <input autoFocus value={replyText} onChange={e => setReplyText(e.target.value)}
              ...
              className={`flex-1 p-4 rounded-2xl text-sm border outline-none transition-all focus:ring-2 ${darkMode?'bg-[#0F0A20] border-[#2A1F55] text-white focus:border-[#8B5CF6] focus:ring-[#8B5CF6]/20 placeholder-[#4C1D95]':'bg-[#F3EEF9] border-[#E9DEFF] focus:border-[#8B5CF6] focus:ring-[#8B5CF6]/10'}`}/>
            <button onClick={handleSendReply} className="px-6 rounded-2xl font-black text-[10px] uppercase tracking-widest text-white btn-magic">
              Send
            </button>
          </div>
        )}
        ...
        {comment.replies.length > 0 && (
          <div className={`mt-8 space-y-6 pl-6 border-l-2 ${darkMode?'border-[#2A1F55]':'border-[#E9DEFF]'}`}>
            {comment.replies.map(reply => (
              <div key={reply.id} className="group/reply relative pl-4">
                <div className="flex justify-between items-start mb-1">
                  <span className={`font-black text-[11px] uppercase tracking-tighter opacity-60 ${subtleText}`}>{reply.author}</span>
                ...
                <p className={`text-sm font-medium leading-relaxed opacity-80 ${darkMode?'text-[#C4B5FD]':'text-[#6D28D9]'}`}>{reply.text}</p>
              </div>
            ))}
          </div>
        )}
...
const Comments = ({ darkMode }) => {
  ...
  const wrapperClass = darkMode ? 'bg-[#0F0A20] text-[#F3EEF8]' : 'bg-[#FAF7FF] text-[#2E1065]';
  const dimColor = darkMode ? 'text-[#A78BFA]' : 'text-[#8B5CF6]';

  if (!data) {
    return (
      <div className={`min-h-screen flex items-center justify-center ${wrapperClass}`}>
  ...
  const handleAddComment = () => {
    ...
    setComments([{ id: Date.now(), author: currentUser, text: newComment, replies: [] }, ...comments]);
    setNewComment("");
  };
  ...
  const handleReply = (commentId, txt) => {
    if (!txt.trim()) return;
    setComments(comments.map(c =>
      c.id === commentId ? { ...c, replies: [...c.replies, { id: Date.now(), author: currentUser, text: txt }] } : c
    ));
  };

  return (
    <div className={`min-h-screen transition-all duration-500 ${wrapperClass}`}>
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div className={`absolute -top-32 left-1/3 w-80 h-80 rounded-full blur-[80px] ${darkMode?'bg-[#8B5CF6]/07':'bg-[#8B5CF6]/04'}`}/>
      </div>
      ...
      <div className="max-w-4xl mx-auto pt-28 pb-10 px-6">
        <button onClick={() => navigate(-1)} className={`group flex items-center gap-2 mb-6 text-sm font-black uppercase tracking-widest opacity-40 hover:opacity-100 transition-all ${dimColor}`}>
      ...
      <div className="max-w-4xl mx-auto px-6 mb-16">
        <div className={`p-2 rounded-[2rem] border shadow-2xl transition-all ${darkMode?'bg-[#1E1540] border-[#2A1F55]':'bg-white border-[#E9DEFF]'}`}
          style={{boxShadow:darkMode?'0 0 40px rgba(109,40,217,.1)':'0 8px 40px rgba(109,40,217,.08)'}}>
          <textarea rows="2" value={newComment} onChange={e => setNewComment(e.target.value)}
            placeholder="Join the conversation..."
            className={`w-full p-6 rounded-[1.5rem] outline-none resize-none font-medium transition-colors ${darkMode?'bg-[#0F0A20] text-white placeholder-[#4C1D95] focus:bg-[#150E2B]':'bg-[#F3EEF9] text-[#2E1065] placeholder-[#A78BFA] focus:bg-white'}`}/>
      ...
      <div className="max-w-4xl mx-auto px-6 pb-32 space-y-10">
        {comments.map(comment => (
          <CommentCard key={comment.id} comment={comment} currentUser={currentUser} darkMode={darkMode}
