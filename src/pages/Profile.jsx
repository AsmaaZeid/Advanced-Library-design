...
const Profile = ({ darkMode }) => {
  ...
  const user = { name:"Alex Johnson", initials:"AJ", joined:"Mar 2025", booksRead:25, rank:"Book Lover", monthly:3, pages:"1,240", favGenre:"Fantasy" };
  ...
  return (
    <div className={`min-h-screen pb-16 ${bg}`}>
      <nav className={`fixed top-0 w-full z-50 backdrop-blur-xl border-b ${dm?'bg-[#0F0A20]/90 border-[#2A1F55]':'bg-white/90 border-[#E9DEFF] shadow-sm'}`}>
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <span className={`text-sm font-bold ${muted}`}>My Profile</span>
        </div>
      </nav>

      <main className="max-w-7xl mx-auto pt-24 px-6 space-y-8">
        {/* Header card */}
        <div className={`p-8 rounded-3xl border ${card}`} style={{ boxShadow:dm?'0 0 50px rgba(109,40,217,.1)':'0 8px 40px rgba(109,40,217,.08)' }}>
          <div className="flex flex-col md:flex-row items-center gap-6">
            ...
            <div className="text-center md:text-left flex-1">
              ...
              <div className="flex flex-wrap gap-3 justify-center md:justify-start text-xs">
                <span className={muted}>✦ Joined {user.joined}</span>
                <span className={muted}>✦ {user.booksRead} Books Read</span>
        ...
        {/* Stats */}
        <div>
          <p className={`${lbl} mb-4`}>✦ Reading Stats</p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              ...
              <div key={i} className={`p-7 rounded-2xl border transition-all hover:scale-[1.02] ${card}`}>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xl" style={{ animation:`float ${3+i*.5}s ${i*.5}s ease-in-out infinite` }}>{s.icon}</span>
                  <span className={`text-xs ${muted}`}>{s.l}</span>
                </div>
                <div className="text-4xl font-black text-shimmer">{s.v}</div>
                <div className={`text-xs mt-1 ${muted}`}>{s.sub}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Borrowed */}
        <div>
          <p className={`${lbl} mb-4`}>✦ Currently Borrowed</p>
          {borrowed.length===0
            ? <div className={`p-8 rounded-2xl text-center border ${card}`}><p className="text-sm text-[#A78BFA]">No books currently borrowed.</p></div>
            : <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {borrowed.map(b=>(
                  <div key={b.id} className={`p-5 rounded-2xl border flex items-center gap-4 ${card}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className={`text-xs mb-1 ${muted}`}>Due: {b.dueDate}</p>
                      <div className="flex text-yellow-400 text-xs">{Array(5).fill(0).map((_,i)=><span key={i}>{i<Math.floor(b.rating)?'★':'☆'}</span>)}</div>
                    </div>
                    <button onClick={()=>setBorrowed(p=>p.filter(x=>x.id!==b.id))} className="px-3 py-2 rounded-xl text-xs font-bold text-white btn-magic hover:-translate-y-0.5 transition-all">Return</button>
                  </div>
                ))}
              </div>
          }
        ...
        {/* Achievements */}
        <div>
          <p className={`${lbl} mb-4`}>✦ Achievements</p>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              {icon:'🏆',l:'First Book',earned:true},{icon:'✦',l:'10 Books Read',earned:true},
              {icon:'★',l:'Top Reviewer',earned:false},{icon:'♥',l:'Community Leader',earned:false},
            ].map((a,i)=>(
              <div key={i} className={`p-5 rounded-2xl border text-center transition-all ${
                a.earned
                  ?(dm?'bg-[#1E1540] border-[#A78BFA]/30':'bg-white border-[#C4B5FD]/50 shadow-md')
                  :(dm?'bg-[#150E2B] border-[#2A1F55] opacity-40':'bg-[#F3EEF9] border-[#E9DEFF] opacity-40')
              }`}>
                ...
                {a.earned&&<span className={`text-[10px] font-bold ${muted}`}>✓ Earned</span>}
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Mascot banner */}
        <div className={`p-8 rounded-3xl border text-center ${card}`}>
          ...
          <p className={`text-sm italic ${dm?'text-[#C4B5FD]':'text-[#6D28D9]'}`} style={{ fontFamily:"'Lora',serif" }}>"Keep reading — Owly and Wormy are cheering for you! ✨"</p>
        </div>
      </main>
    </div>
  );
