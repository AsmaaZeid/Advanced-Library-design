...
const DisplayComm = ({ darkMode }) => {
  ...
  const inp = `w-full px-4 py-3 rounded-xl border outline-none text-sm focus:ring-2 transition-all ${dm?'bg-[#0F0A20] border-[#2A1F55] text-white focus:border-[#8B5CF6] focus:ring-[#8B5CF6]/20 placeholder-[#4C1D95]':'bg-[#F3EEF9] border-[#E9DEFF] focus:border-[#8B5CF6] focus:ring-[#8B5CF6]/10 placeholder-[#A78BFA]'}`;

  const filtered = communities.filter(c => c.name.toLowerCase().includes(search.toLowerCase()));

  const handleCreate = (e) => {
    e.preventDefault(); setError("");
    if (communities.some(c => c.name.toLowerCase() === newComm.name.toLowerCase())) { setError("A community with this name already exists!"); return; }
    const created = { id: communities.length + 1, ...newComm, admin: currentUser, members: [currentUser] };
    setCommunities([created, ...communities]);
    setShowModal(false); setNewComm({ name:"", description:"", category:"" });
    navigate(`/groups/${created.id}`, { state: created });
  };

  return (
    <div className={`min-h-screen pb-16 ${bg} relative overflow-x-hidden`}>
      {/* Soft bg glow */}
        ...
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between gap-3">
          ...
          <div className="relative flex-1 max-w-sm">
            ...
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search communities..."
              className={`w-full pl-9 pr-4 py-2 rounded-xl border text-sm outline-none focus:ring-2 transition-all ${dm?'bg-[#1E1540] border-[#2A1F55] text-white placeholder-[#6D28D9] focus:border-[#8B5CF6] focus:ring-[#8B5CF6]/20':'bg-white border-[#E9DEFF] placeholder-[#A78BFA] focus:border-[#8B5CF6] focus:ring-[#8B5CF6]/10'}`}/>
          </div>
      ...
      <main className="relative max-w-6xl mx-auto px-6 pt-12">
        ...
        <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <p className={`text-xs uppercase tracking-[.3em] font-bold mb-2 ${muted}`}>✦ Connect with readers</p>
            <h1 style={{ fontFamily:"'Playfair Display',serif" }} className="text-4xl font-black mb-1">Reader Communities</h1>
            <p className={`text-sm ${muted}`}>{filtered.length} communities waiting for you</p>
          </div>
          {/* Mini characters */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {filtered.map(comm => {
            ...
            const gradients = {
              Fantasy:'from-[#6D28D9] to-[#8B5CF6]', Academic:'from-[#1D4ED8] to-[#3B82F6]',
              Mystery:'from-[#374151] to-[#6B7280]', Romance:'from-[#BE185D] to-[#EC4899]',
              'Science Fiction':'from-[#0E7490] to-[#06B6D4]', Children:'from-[#D97706] to-[#F59E0B]', default:'from-[#6D28D9] to-[#EC4899]'
            };
            const grad = gradients[comm.category] || gradients.default;
            return (
              <div key={comm.id}
                className={`group p-6 rounded-3xl border transition-all duration-300 hover:-translate-y-2 cursor-pointer ${dm?`${card} hover:border-[#A78BFA]`:`${card} hover:border-[#8B5CF6] shadow-sm hover:shadow-xl`}`}
                style={{ boxShadow: dm?'0 0 0 transparent':'' }}
                ...
                <div className="flex items-start justify-between mb-5">
                  ...
                  {isMember&&(
                    <span className={`text-[10px] px-2.5 py-1 rounded-full font-bold uppercase tracking-wider ${dm?'bg-[#8B5CF6]/20 text-[#A78BFA]':'bg-[#F3EEF9] text-[#6D28D9]'}`}>✓ Joined</span>
                  )}
                </div>

                {/* Name & desc */}
                <h3 style={{ fontFamily:"'Playfair Display',serif" }} className="font-bold text-xl mb-2 leading-tight">{comm.name}</h3>
                <p className={`text-sm leading-relaxed mb-5 ${dm?'text-[#C4B5FD]':'text-[#6D28D9]'}`}>{comm.description}</p>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex -space-x-2">
                      {comm.members.slice(0,3).map((m,i)=>(
                        <div key={i} className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold text-white border-2 bg-gradient-to-br ${grad} ${dm?'border-[#1E1540]':'border-white'}`}>{m[0]}</div>
                      ))}
                    </div>
                    <span className={`text-xs font-medium ${dm?'text-[#A78BFA]':'text-[#8B5CF6]'}`}>{comm.members.length} members</span>
                  </div>
                  <span className={`text-xs font-bold group-hover:translate-x-1 transition-transform ${dm?'text-[#A78BFA]':'text-[#8B5CF6]'}`}>Enter ›</span>
                </div>

                {/* Category tag */}
                <div className="mt-4">
                  <span className={`text-[10px] px-2.5 py-1 rounded-full font-semibold ${dm?'bg-[#2A1F55] text-[#C4B5FD]':'bg-[#F3EEF9] text-[#6D28D9]'}`}>{comm.category}</span>
                </div>
              </div>
            );
          })}
        ...
        {filtered.length===0&&(
          <div className="text-center py-20">
            ...
            <p className={`text-sm ${muted}`}>Try a different search or create one!</p>
          </div>
        )}
      </main>
      ...
      {showModal&&(
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className={`w-full max-w-md p-8 rounded-3xl border shadow-2xl animate-zoom-in ${dm?'bg-[#1E1540] border-[#2A1F55]':'bg-white border-[#E9DEFF]'}`}
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={()=>setShowModal(false)} className={`w-9 h-9 rounded-xl flex items-center justify-center hover:rotate-90 transition-all ${dm?'bg-[#2A1F55] text-[#C4B5FD]':'bg-[#F3EEF9] text-[#6D28D9]'}`}>✕</button>
            </div>
            <form onSubmit={handleCreate} className="space-y-4">
              {[
                ...
                <div key={f.name}>
                  <label className={`block text-[10px] uppercase tracking-widest font-bold mb-1.5 ${muted}`}>{f.label}</label>
                  <input type="text" placeholder={f.ph} value={newComm[f.name]}
                    onChange={e=>setNewComm(p=>({...p,[f.name]:e.target.value}))} required className={inp}/>
                </div>
              ))}
