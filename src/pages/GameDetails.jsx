...
const StatCard = ({ icon, label, value, darkMode }) => (
  <div className={`p-5 rounded-2xl border flex items-center gap-4 transition-all hover:scale-[1.02] ${darkMode?'bg-[#2A1F55]/40 border-[#2A1F55]':'bg-[#F3EEF9] border-[#E9DEFF]'}`}>
    ...
    <div>
      <p className={`text-[10px] uppercase tracking-widest font-bold mb-0.5 ${darkMode?'text-[#A78BFA]':'text-[#8B5CF6]'}`}>{label}</p>
...
const GameDetails = ({ darkMode }) => {
  ...
  return (
    <div className={`min-h-screen pb-20 transition-colors duration-500 ${pageStyle}`}>
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div className={`absolute -top-32 -left-20 w-80 h-80 rounded-full blur-[80px] ${isDark?'bg-[#8B5CF6]/08':'bg-[#8B5CF6]/05'}`}/>
        <div className={`absolute -bottom-32 -right-20 w-80 h-80 rounded-full blur-[80px] ${isDark?'bg-[#EC4899]/06':'bg-[#EC4899]/04'}`}/>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 -mt-12 relative z-20">
        <div className="grid lg:grid-cols-3 gap-8">
          <div className={`lg:col-span-2 p-8 md:p-10 rounded-3xl border backdrop-blur-2xl shadow-2xl ${isDark?'bg-[#1E1540]/85 border-[#2A1F55]':'bg-white/92 border-[#E9DEFF]'}`}
            style={{ boxShadow: isDark?'0 20px 60px rgba(109,40,217,.15)':'0 20px 60px rgba(109,40,217,.08)' }}>
            <p className={`text-xs uppercase tracking-[.3em] font-bold mb-2 ${isDark?'text-[#A78BFA]':'text-[#8B5CF6]'}`}>✦ About this Journey</p>
            <h2 style={{ fontFamily:"'Playfair Display',serif" }} className="text-3xl font-black mb-5">Path Overview</h2>
            <p className={`text-base leading-relaxed mb-8 ${isDark?'text-[#C4B5FD]':'text-[#6D28D9]'}`}>{game.description}</p>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
              <StatCard darkMode={isDark} label="Learners" value={members.toLocaleString()+'+'} icon={
              ...
              <StatCard darkMode={isDark} label="Level" value={game.difficulty} icon={
              ...
              <StatCard darkMode={isDark} label="Duration" value={game.duration} icon={
            ...
            <p className={`text-xs uppercase tracking-[.3em] font-bold mb-4 ${isDark?'text-[#A78BFA]':'text-[#8B5CF6]'}`}>✦ Core Competencies</p>
            <div className="flex flex-wrap gap-3">
              {game.topics.map((t, i) => (
                <span key={i} className={`px-4 py-2 rounded-xl text-sm font-semibold border transition-all hover:scale-105 ${isDark?'bg-[#2A1F55]/60 border-[#2A1F55] text-[#C4B5FD]':'bg-[#F3EEF9] border-[#E9DEFF] text-[#6D28D9]'}`}>{t}</span>
              ))}
            </div>
          </div>

          <div className="space-y-5">
            <div className={`p-8 rounded-3xl border shadow-xl ${isDark?'bg-[#1E1540] border-[#2A1F55]':'bg-white border-[#E9DEFF]'}`}
              style={{ boxShadow: isDark?'0 0 40px rgba(109,40,217,.12)':'0 8px 40px rgba(109,40,217,.08)' }}>
              <p className={`text-xs uppercase tracking-[.25em] font-bold mb-2 ${isDark?'text-[#A78BFA]':'text-[#8B5CF6]'}`}>✦ Ready to Start?</p>
              <h3 style={{ fontFamily:"'Playfair Display',serif" }} className="text-xl font-black mb-3">Join this Path</h3>
              <p className={`text-sm leading-relaxed mb-6 ${isDark?'text-[#C4B5FD]':'text-[#6D28D9]'}`}>Join thousands of others tracking their progress in this path.</p>
              ...
              <button onClick={() => navigate(`/roadmap/${id}`)}
                className={`w-full py-4 rounded-2xl font-bold text-sm border transition-all hover:-translate-y-0.5 active:scale-95 ${isDark?'bg-[#2A1F55]/60 border-[#2A1F55] text-[#C4B5FD] hover:bg-[#2A1F55]':'bg-[#F3EEF9] border-[#E9DEFF] text-[#6D28D9] hover:bg-[#EDE5FF]'}`}>
                ✦ Open Roadmap
              </button>
            </div>

            <div className={`p-8 rounded-3xl border overflow-hidden relative ${isDark?'bg-gradient-to-br from-[#6D28D9]/20 to-[#EC4899]/10 border-[#2A1F55]':'bg-gradient-to-br from-[#F3EEF9] to-[#FFF0F7] border-[#E9DEFF]'}`}>
              <div className="relative z-10">
                ...
                <p className={`text-xs leading-relaxed ${isDark?'text-[#A78BFA]':'text-[#8B5CF6]'}`}>Earn a badge upon completion of all modules.</p>
              </div>
