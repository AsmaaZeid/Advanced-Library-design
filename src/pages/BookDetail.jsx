...
/* ── Book opening splash — purple themed ── */
...
const BookDetail = ({ darkMode }) => {
  ...
  useEffect(() => { if(isRandom){ const t=setTimeout(()=>setShowDiceAnimation(false),1400); return()=>clearTimeout(t); } }, [isRandom]);

  const data = book || {
    id:1, title:"The Midnight Library", author:"Matt Haig", rating:4.82,
    description:"Between life and death there is a library, and within that library, the shelves go on forever. Every book provides a chance to try another life you could have lived.",
    copiesleft:5, publisher:"Penguin Books", placeOfPublish:"London, UK", language:"English",
    ISBN:"978-0525559474", genre:"Fantasy", type:"Hardcover", pages:304, publicationDate:"August 13, 2020",
  };

  const moreBooks = [
    {id:101,title:"How to Stop Time",year:2017},{id:102,title:"The Humans",year:2013},
    {id:103,title:"Notes on a Nervous Planet",year:2018},{id:104,title:"The Comfort Book",year:2021},
  ];

  const dm = darkMode;
  const bg = dm?'bg-[#0F0A20] text-[#F3EEF8]':'bg-[#FAF7FF] text-[#2E1065]';
  const card = dm?'bg-[#1E1540] border-[#2A1F55]':'bg-white border-[#E9DEFF] shadow-lg';

  return (
    <div className={`min-h-screen transition-colors duration-500 ${bg}`}>
      {showSplash&&<BookSplash darkMode={dm} title={data.title} author={data.author} onDone={()=>setShowSplash(false)}/>}

      {/* NAVBAR */}
      <nav className={`fixed top-0 left-0 right-0 z-[100] backdrop-blur-xl border-b transition-all ${dm?'bg-[#0F0A20]/90 border-[#2A1F55]':'bg-white/90 border-[#E9DEFF] shadow-sm'}`}>
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-6">
            ...
            <span className={`hidden md:block px-3 py-1 rounded-full text-xs font-bold ${dm?'bg-[#8B5CF6]/20 text-[#A78BFA]':'bg-[#F3EEF9] text-[#6D28D9]'}`}>{data.genre}</span>
          </div>
          <Link to={`/comments/book/${data.id}`} state={{type:"book",data}}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold border transition-all hover:-translate-y-0.5 ${dm?'border-[#2A1F55] text-[#C4B5FD] hover:bg-[#1E1540]':'border-[#E9DEFF] text-[#6D28D9] hover:bg-[#F3EEF9]'}`}>
            💬 Discussion
          </Link>
        </div>
      </nav>
      ...
      {/* Background glow */}
      <div className={`fixed top-0 left-0 w-full h-[500px] opacity-[0.07] pointer-events-none blur-[100px] ${dm?'bg-[#8B5CF6]':'bg-[#6D28D9]'}`}/>
        ...
        {showDiceAnimation&&(
          <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
            ...
            <motion.div initial={{scale:.7,rotate:0}} animate={{scale:[.7,1.15,1],rotate:[0,180,360,540]}} exit={{scale:.8,opacity:0}} transition={{duration:1.1,ease:"easeInOut"}}
              className={`w-24 h-24 rounded-3xl shadow-2xl flex items-center justify-center text-4xl font-bold border-4 ${dm?'bg-[#1E1540] border-[#A78BFA] text-white':'bg-white border-[#6D28D9] text-[#2E1065]'}`}>
              ✧
            </motion.div>
          </motion.div>
        )}
      ...
      <motion.main initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:.5,delay:.5}}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
          ...
          <div className="lg:col-span-4 space-y-8">
            <div className={`group rounded-[2rem] overflow-hidden shadow-2xl border-8 transition-all hover:rotate-1 ${dm?'border-[#1E1540]':'border-white'}`}
              style={{boxShadow:dm?'0 20px 60px rgba(109,40,217,.25)':'0 20px 60px rgba(109,40,217,.12)'}}>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-2 gap-3 mt-2">
                <ShelfBtn icon="✓"  text="Read"         darkMode={dm}/>
                <ShelfBtn icon="📖" text="Reading"      darkMode={dm}/>
                <ShelfBtn icon="🔖" text="Want to Read" darkMode={dm}/>
                <ShelfBtn icon="♥"  text="Favorite"     darkMode={dm}/>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-8 space-y-10">
            <section>
              ...
              <div className="flex items-center gap-6">
                ...
                <div className={`px-4 py-1.5 rounded-xl flex items-center font-black ${dm?'bg-yellow-500/10 text-yellow-400':'bg-yellow-50 text-yellow-600'}`}>
                  <span className="mr-1">★</span>{Number(data.rating).toFixed(1)}
                </div>
              </div>
            </section>

            <div className={`p-8 rounded-[2rem] border transition-all ${card}`}>
              <p className={`text-xs uppercase tracking-widest font-black mb-4 ${dm?'text-[#A78BFA]':'text-[#8B5CF6]'}`}>✦ Synopsis</p>
              <p className="text-lg leading-relaxed opacity-90">{data.description}</p>
            </div>

            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              <StatCard label="Publisher"  value={data.publisher}       icon="🏢" darkMode={dm}/>
              <StatCard label="Date"       value={data.publicationDate} icon="📅" darkMode={dm}/>
              <StatCard label="Location"   value={data.placeOfPublish}  icon="📍" darkMode={dm}/>
              <StatCard label="ISBN"       value={data.ISBN}            icon="🔢" darkMode={dm}/>
              <StatCard label="Language"   value={data.language}        icon="🌐" darkMode={dm}/>
              <StatCard label="Type"       value={data.type}            icon="📚" darkMode={dm}/>
              <StatCard label="Pages"      value={data.pages}           icon="📄" darkMode={dm}/>
              <StatCard label="Stock"      value={`${data.copiesleft} Left`} icon="📦" darkMode={dm}
                color={data.copiesleft>0?"text-emerald-500":"text-red-400"}/>
            </div>

            <div className={`p-8 rounded-[2rem] border-2 border-dashed flex flex-col md:flex-row items-center justify-between gap-6 ${dm?'border-[#A78BFA]/25 bg-[#8B5CF6]/5':'border-[#C4B5FD]/50 bg-[#F3EEF9]'}`}>
              <div>
                <h3 className="text-2xl font-black mb-1">Your Review</h3>
                <p className={`opacity-60 text-sm ${dm?'text-[#C4B5FD]':'text-[#6D28D9]'}`}>Help others discover this book</p>
              </div>
              <div className="flex flex-col items-center gap-3">
                ...
                {userRating>0&&<div className="flex text-yellow-400 text-2xl">{[...Array(5)].map((_,i)=><span key={i}>{i<userRating?"★":"☆"}</span>)}</div>}
              </div>
            </div>
          </div>
        </div>
        ...
        {/* More by author */}
        <section className={`pt-10 border-t ${dm?'border-[#2A1F55]':'border-[#E9DEFF]'}`}>
          <h2 style={{fontFamily:"'Playfair Display',serif"}} className="text-3xl font-black mb-8">More Work by {data.author}</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {moreBooks.map(b=>(
              <div key={b.id} className={`p-6 rounded-3xl border transition-all hover:-translate-y-2 cursor-pointer ${card}`}>
                <div className={`aspect-[3/4] rounded-xl mb-4 flex items-center justify-center ${dm?'bg-[#2A1F55]':'bg-[#F3EEF9]'}`}>
                ...
                <p className={`text-xs mt-1 ${dm?'text-[#A78BFA]':'text-[#8B5CF6]'}`}>{b.year}</p>
              </div>
            ))}
          </div>
        </section>
      ...
      {showRateModal&&(
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm">
          <div className={`w-full max-w-sm p-10 rounded-[2.5rem] text-center border animate-zoom-in ${dm?'bg-[#1E1540] border-[#2A1F55]':'bg-white border-[#E9DEFF]'}`}
            style={{boxShadow:dm?'0 0 60px rgba(109,40,217,.3)':'0 20px 60px rgba(109,40,217,.12)'}}>
            ...
            <button onClick={()=>setShowRateModal(false)} className={`text-sm font-bold opacity-50 ${dm?'text-[#C4B5FD]':'text-[#6D28D9]'}`}>Cancel</button>
          </div>
        </div>
      )}
