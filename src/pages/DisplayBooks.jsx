...
/* ── Book genres for sidebar filter ── */
  ...
  return (
    <Link to={`/book/${book.id}`} state={{ book }}>
      <div className={`group flex flex-col rounded-2xl transition-all duration-300 book-card-hover border p-3 overflow-hidden ${
        dm ? 'bg-[#1E1540] border-[#2A1F55] hover:border-[#A78BFA]' : 'bg-white border-[#E9DEFF] hover:border-[#8B5CF6] shadow-sm hover:shadow-lg'
      }`}>
        ...
        <h3 className={`font-bold truncate text-sm mb-1 ${dm?'text-[#F3EEF8]':'text-[#2E1065]'}`}>{book.title}</h3>
...
const DisplayBooks = ({ darkMode }) => {
  ...
  const handleGenre = g => { setActiveGenre(g); setCurrentPage(1); };

  const inp = `w-full px-4 py-2.5 rounded-xl border outline-none text-sm focus:ring-2 transition-all ${dm?'bg-[#1E1540] border-[#2A1F55] text-white focus:border-[#8B5CF6] focus:ring-[#8B5CF6]/20 placeholder-[#6D28D9]':'bg-white border-[#E9DEFF] text-[#2E1065] focus:border-[#8B5CF6] focus:ring-[#8B5CF6]/10 placeholder-[#A78BFA]'}`;

  return (
    <div className={`min-h-screen pb-16 transition-colors ${bg}`}>
      {/* Soft bg glow */}
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div className={`absolute -top-32 -left-32 w-80 h-80 rounded-full blur-[80px] ${dm?'bg-[#8B5CF6]/08':'bg-[#8B5CF6]/05'}`}/>
        <div className={`absolute -bottom-32 -right-32 w-80 h-80 rounded-full blur-[80px] ${dm?'bg-[#EC4899]/06':'bg-[#EC4899]/04'}`}/>
      </div>

      {/* ── NAVBAR ── */}
      <nav className={`sticky top-0 z-50 border-b backdrop-blur-xl ${dm?'bg-[#0F0A20]/92 border-[#2A1F55]':'bg-[#FAF7FF]/92 border-[#E9DEFF]'}`}>
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between gap-3">
          ...
          <Link to="/" onClick={() => sessionStorage.setItem('showBookIntro', 'true')} className="flex items-center gap-2 group flex-shrink-0">
            ...
            <span style={{fontFamily:"'Playfair Display',serif"}} className={`font-black text-xl italic hidden sm:block ${dm?'text-[#C4B5FD]':'text-[#4C1D95]'}`}>BiblioTech</span>
          </Link>
          ...
          <button onClick={()=>setSidebarOpen(!sidebarOpen)}
            className={`md:hidden flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center border ${dm?'bg-[#1E1540] border-[#2A1F55] text-[#C4B5FD]':'bg-white border-[#E9DEFF] text-[#6D28D9]'}`}>
            ⚙
          </button>

          <span className={`text-xs font-medium flex-shrink-0 hidden md:block ${muted}`}>{filtered.length} books</span>
        </div>
      </nav>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 pt-8 flex gap-8">
        ...
        <aside className={`hidden md:flex flex-col w-56 flex-shrink-0 space-y-6`}>
          ...
          <div className={`p-5 rounded-2xl border ${card}`}>
            <p className={`text-[10px] uppercase tracking-widest font-black mb-4 ${muted}`}>✦ Genre</p>
                ...
                <button key={g} onClick={()=>handleGenre(g)}
                  className={`w-full text-left px-3 py-2 rounded-xl text-sm font-medium transition-all ${
                    activeGenre===g
                      ...
                      : dm?'text-[#C4B5FD] hover:bg-[#2A1F55]':'text-[#6D28D9] hover:bg-[#F3EEF9]'
                  }`}>
                  {g}
                </button>
              ))}
          ...
          {/* Sort */}
          <div className={`p-5 rounded-2xl border ${card}`}>
            <p className={`text-[10px] uppercase tracking-widest font-black mb-3 ${muted}`}>✦ Sort By</p>
            {[{v:'year',l:'Newest First'},{v:'rating',l:'Top Rated'},{v:'price',l:'Price: Low'},{v:'copies',l:'Most Available'}].map(s=>(
              <button key={s.v} onClick={()=>setSortBy(s.v)}
                className={`w-full text-left px-3 py-2 rounded-xl text-sm font-medium transition-all mb-1 ${
                  sortBy===s.v
                    ? dm?'bg-[#2A1F55] text-[#A78BFA] font-bold':'bg-[#F3EEF9] text-[#6D28D9] font-bold'
                    : dm?'text-[#C4B5FD] hover:bg-[#2A1F55]':'text-[#6D28D9] hover:bg-[#F3EEF9]'
                }`}>
                {s.l}
              </button>
          ...
          {/* Availability */}
          <div className={`p-5 rounded-2xl border ${card}`}>
            <p className={`text-[10px] uppercase tracking-widest font-black mb-3 ${muted}`}>✦ Availability</p>
            <label className="flex items-center gap-3 cursor-pointer group">
              <div onClick={()=>setAvailableOnly(!availableOnly)}
                className={`w-11 h-6 rounded-full transition-all duration-300 relative flex-shrink-0 ${availableOnly?'bg-gradient-to-r from-[#6D28D9] to-[#EC4899]':dm?'bg-[#2A1F55]':'bg-[#E9DEFF]'}`}>
              ...
              <span className={`text-sm font-medium ${dm?'text-[#C4B5FD]':'text-[#6D28D9]'}`}>Available only</span>
            </label>
          </div>

          {/* Reset */}
          {(activeSearch||activeGenre!=='All'||availableOnly||sortBy!=='year')&&(
            <button onClick={()=>{ setActiveSearch(''); setSearchInput(''); setActiveGenre('All'); setAvailableOnly(false); setSortBy('year'); setCurrentPage(1); }}
              className={`w-full py-2.5 rounded-xl text-sm font-bold border transition-all hover:-translate-y-0.5 ${dm?'border-[#2A1F55] text-[#A78BFA] hover:bg-[#1E1540]':'border-[#E9DEFF] text-[#6D28D9] hover:bg-[#F3EEF9]'}`}>
              Reset Filters
            </button>
          )}
        </aside>
        ...
        {sidebarOpen&&(
          <div className="md:hidden fixed inset-0 z-[80] flex">
            ...
            <div className={`relative ml-auto w-72 h-full overflow-y-auto p-6 space-y-5 ${dm?'bg-[#0F0A20]':'bg-[#FAF7FF]'}`}>
              <div className="flex justify-between items-center mb-2">
                <p className={`font-black text-lg ${muted}`}>Filters</p>
                <button onClick={()=>setSidebarOpen(false)} className="text-2xl opacity-50">✕</button>
              </div>
              <div className={`p-4 rounded-2xl border ${card}`}>
                <p className={`text-[10px] uppercase tracking-widest font-black mb-3 ${muted}`}>Genre</p>
                <div className="flex flex-wrap gap-2">
                  {GENRES.map(g=>(
                    <button key={g} onClick={()=>{handleGenre(g);setSidebarOpen(false);}}
                      className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all ${activeGenre===g?'btn-magic text-white':dm?'bg-[#2A1F55] text-[#C4B5FD]':'bg-[#F3EEF9] text-[#6D28D9]'}`}>
                      {g}
                    </button>
                  ))}
                </div>
              ...
              <div className={`p-4 rounded-2xl border ${card}`}>
                ...
                {[{v:'year',l:'Newest'},{v:'rating',l:'Top Rated'},{v:'price',l:'Price'},{v:'copies',l:'Available'}].map(s=>(
                  <button key={s.v} onClick={()=>{setSortBy(s.v);setSidebarOpen(false);}}
                    className={`w-full text-left px-3 py-2 rounded-xl text-sm mb-1 transition-all ${sortBy===s.v?(dm?'bg-[#2A1F55] text-[#A78BFA] font-bold':'bg-[#F3EEF9] text-[#6D28D9] font-bold'):dm?'text-[#C4B5FD]':'text-[#6D28D9]'}`}>
                    {s.l}
                  </button>
                ))}
              </div>
        ...
        <main className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className={`text-sm mt-1 ${muted}`}>{filtered.length} books found</p>
            </div>
          </div>

          {current.length===0 ? (
            <div className={`flex flex-col items-center justify-center py-20 rounded-3xl border ${card}`}>
              ...
              <p className={`text-sm ${muted}`}>Try different search terms or filters.</p>
            </div>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
              {current.map(book=><BookCard key={book.id} book={book} darkMode={dm}/>)}
            </div>
          )}
          ...
          {totalPages>1&&(
            <div className="mt-12 flex items-center justify-center gap-3">
              <button onClick={()=>setCurrentPage(p=>Math.max(p-1,1))} disabled={currentPage===1}
                className={`px-4 py-2.5 rounded-xl font-bold text-sm border transition-all disabled:opacity-30 hover:-translate-y-0.5 ${dm?'bg-[#1E1540] border-[#2A1F55] text-white':'bg-white border-[#E9DEFF] text-[#2E1065]'}`}>
                ← Prev
              </button>
                ...
                {[...Array(Math.min(totalPages,5))].map((_,i)=>{
                  ...
                  return (
                    <button key={page} onClick={()=>setCurrentPage(page)}
                      className={`w-10 h-10 rounded-xl font-bold text-sm transition-all ${currentPage===page?'btn-magic text-white':dm?'bg-[#1E1540] border border-[#2A1F55] text-[#C4B5FD]':'bg-white border border-[#E9DEFF] text-[#6D28D9]'}`}>
                      {page}
                    </button>
                  );
                })}
