...
const CommunityDetail = ({ darkMode }) => {
  ...
  const [community, setCommunity] = useState(location.state || {
    id:0, name:"General Readers", description:"Welcome to the central hub for all book lovers.",
    category:"General", admin:"System", members:["Ali","Sara","John"]
  });
  ...
  const handleJoin  = () => { if(!isMember) setCommunity({...community,members:[...community.members,currentUser]}); };
  const handleLeave = () => { if(isMember)  setCommunity({...community,members:community.members.filter(m=>m!==currentUser)}); };
  const removeMember = m => setCommunity({...community,members:community.members.filter(x=>x!==m)});

  return (
    <div className={`min-h-screen transition-colors duration-500 ${bg}`}>
      {/* Soft glow */}
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div className={`absolute -top-32 left-1/3 w-80 h-80 rounded-full blur-[80px] ${dm?'bg-[#8B5CF6]/08':'bg-[#8B5CF6]/05'}`}/>
        <div className={`absolute bottom-0 right-1/4 w-72 h-72 rounded-full blur-[80px] ${dm?'bg-[#EC4899]/06':'bg-[#EC4899]/04'}`}/>
      </div>

      {/* NAVBAR */}
      <nav className={`fixed top-0 left-0 right-0 z-[100] backdrop-blur-xl border-b ${dm?'bg-[#0F0A20]/90 border-[#2A1F55]':'bg-white/90 border-[#E9DEFF] shadow-sm'}`}>
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-8">
            ...
            <Link to="/groups" className={`text-sm font-medium opacity-60 hover:opacity-100 nav-link ${muted}`}>All Communities</Link>
          </div>
        </div>
      </nav>
      ...
      <main className="relative max-w-6xl mx-auto pt-28 pb-20 px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 items-end mb-16">
          <div className="lg:col-span-2 space-y-5">
            <span className={`inline-block text-xs uppercase tracking-[.2em] font-black px-4 py-1.5 rounded-full ${dm?'bg-[#8B5CF6]/20 text-[#A78BFA]':'bg-[#F3EEF9] text-[#6D28D9]'}`}>
              {community.category}
            </span>
            ...
            <p className={`text-xl leading-relaxed max-w-2xl font-medium ${dm?'text-[#C4B5FD]':'text-[#6D28D9]'}`}>{community.description}</p>
          ...
          <div className="space-y-4">
            {!isMember
              ...
              : <button onClick={handleLeave}
                  className={`w-full py-5 rounded-2xl font-black text-center border-2 border-dashed transition-all group hover:border-red-400 hover:text-red-400 hover:bg-red-500/5 ${dm?'border-[#A78BFA]/40 text-[#A78BFA]':'border-[#8B5CF6]/40 text-[#6D28D9]'}`}>
            ...
            <button onClick={()=>navigate(`/comments/groups/${community.id}`,{state:{type:"community",data:community}})}
              className={`w-full py-5 rounded-2xl border-2 font-black text-lg transition-all hover:shadow-xl active:scale-95 hover:-translate-y-0.5 ${dm?'border-[#A78BFA] text-[#A78BFA] hover:bg-[#A78BFA] hover:text-white':'border-[#6D28D9] text-[#6D28D9] hover:bg-[#6D28D9] hover:text-white'}`}>
              Discussion Board ✦
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {/* Members list */}
          <div className={`lg:col-span-2 rounded-[2.5rem] p-10 border transition-all ${card}`}
            style={{boxShadow:dm?'0 0 40px rgba(109,40,217,.08)':'0 8px 40px rgba(109,40,217,.08)'}}>
            <div className="flex justify-between items-center mb-10">
              <h2 style={{fontFamily:"'Playfair Display',serif"}} className="text-3xl font-black">Members List</h2>
              <span className={`text-sm font-bold uppercase tracking-widest opacity-50 ${muted}`}>{community.members.length} total</span>
            </div>
            <div className={`divide-y ${dm?'divide-[#2A1F55]':'divide-[#F3EEF9]'}`}>
              {community.members.map((member,i)=>(
                <div key={i} className="flex justify-between items-center py-5 group transition-all">
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold transition-transform group-hover:scale-110 ${dm?'bg-[#2A1F55] text-[#A78BFA]':'bg-[#F3EEF9] text-[#6D28D9]'}`}>
                      {member[0]}
                    </div>
                    <div>
          ...
          <div className="space-y-6">
            <div className={`p-8 rounded-[2.5rem] border ${card}`}>
              <h3 className={`text-xs font-black uppercase tracking-widest mb-6 ${muted}`}>✦ Hub Rules</h3>
              <ul className="space-y-4 text-sm font-medium">
                {['Be respectful to all members.','No spoilers without tags.','Stay on topic (Mostly).'].map((r,i)=>(
                  <li key={i} className="flex items-start gap-3">
                    <span className={`font-black text-xs ${muted}`}>0{i+1}.</span>
                    <span className={dm?'text-[#C4B5FD]':'text-[#6D28D9]'}>{r}</span>
                  </li>
                ))}
              </ul>
            </div>
            <div className={`p-8 rounded-[2.5rem] border text-center transition-all hover:scale-[1.02] ${dm?'bg-gradient-to-br from-[#1E1540] to-[#2A1F55]/50 border-[#2A1F55]':'bg-gradient-to-br from-[#F3EEF9] to-white border-[#E9DEFF]'}`}>
              ...
              <p className={`text-xs leading-relaxed ${muted}`}>Contact system admins to request additional moderator permissions.</p>
            </div>
          </div>
        </div>
      </main>
