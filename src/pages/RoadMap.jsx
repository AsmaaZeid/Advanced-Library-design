...
const RoadmapBook = ({ book, darkMode }) => {
  ...
  return (
    <div onClick={() => navigate(`/book/${book.id}`, { state:{ book } })}
      ...
      <p className={`font-bold text-xs truncate px-1 ${darkMode?'text-[#F3EEF8]':'text-[#2E1065]'}`}>{book.title}</p>
      <p className={`text-[10px] italic truncate px-1 mt-0.5 ${darkMode?'text-[#A78BFA]':'text-[#8B5CF6]'}`}>{book.author}</p>
    </div>
  );
};
    ...
    <div className="relative flex flex-col items-center">
      <div className={`relative w-full flex flex-col md:flex-row transition-all duration-700 ${!isUnlocked?'opacity-35 grayscale':''}`}>
        <div className={`absolute left-4 md:left-1/2 md:-translate-x-1/2 z-10 w-14 h-14 rounded-2xl flex items-center justify-center border-2 shadow-xl transition-all ${
          isCompleted
            ...
            : `${isDark?'bg-[#1E1540] border-[#2A1F55]':'bg-[#F3EEF9] border-[#E9DEFF]'}`
        }`}>
          {isCompleted ? (
            ...
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={isDark?'#A78BFA':'#8B5CF6'} strokeWidth="2"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
          ) : (
        ...
        <div className={`w-full md:w-[46%] pl-24 md:pl-0 p-7 rounded-3xl border transition-all ${
          isCompleted
            ? isDark ? 'bg-emerald-500/8 border-emerald-500/25' : 'bg-emerald-50 border-emerald-200/60'
            : isDark ? 'bg-[#1E1540] border-[#2A1F55]' : 'bg-white border-[#E9DEFF] shadow-md'
        } ${isEven?'md:mr-auto':'md:ml-auto'}`}
          style={{ boxShadow: isUnlocked && !isCompleted ? (isDark?'0 0 30px rgba(109,40,217,.1)':'0 8px 30px rgba(109,40,217,.07)') : 'none' }}>
          ...
          <div className="flex justify-between items-start mb-5">
            <div>
              ...
              {step.description && <p className={`text-sm mt-1 ${isDark?'text-[#C4B5FD]':'text-[#6D28D9]'}`}>{step.description}</p>}
            </div>
          ...
          <div className="relative group/books">
            ...
            <div ref={scrollRef} className="flex gap-3 overflow-x-auto pb-3 books-scroll">
              {step.books.map(book => <RoadmapBook key={book.id} book={book} darkMode={isDark}/>)}
            </div>
          </div>
        </div>
      </div>
      ...
      {!isLast && (
        <div className={`w-0.5 h-10 rounded-full my-2 transition-all duration-700 ${
          isCompleted ? 'bg-gradient-to-b from-emerald-500 to-[#8B5CF6]' : isDark ? 'bg-[#2A1F55]' : 'bg-[#E9DEFF]'
        }`}/>
      )}
    </div>
  );
...
const RoadMap = ({ darkMode }) => {
  ...
  const roadmapSteps = [
    { id:1, level:"Beginner", title:"Foundations", description:"Start with the building blocks every developer needs.", books:[
      {id:1,title:"HTML & CSS Basics",author:"Jon Duckett"},
      {id:2,title:"JavaScript Intro",author:"Marijn Haverbeke"},
      {id:3,title:"Web Design 101",author:"Jason Beaird"},
    ]},
    ...
    { id:3, level:"Intermediate", title:"React & State", description:"Build dynamic UIs with the world's most popular UI library.", books:[
      {id:7,title:"Learning React",author:"Alex Banks"},
      {id:8,title:"React Design Patterns",author:"Michele Bertoli"},
    ]},
    { id:4, level:"Advanced", title:"Full Stack & Beyond", description:"Connect frontend to backend and ship real applications.", books:[
      {id:9,title:"Node.js in Action",author:"Mike Cantelon"},
      {id:10,title:"Designing Data Apps",author:"Martin Kleppmann"},
    ]},
  ];
    ...
    <div className={`min-h-screen ${pageStyle}`}>
      ...
      <div className="relative pt-16">
        <div className="relative h-52 md:h-64 overflow-hidden">
          <div className="absolute inset-0" style={{ background:isDark?'linear-gradient(135deg,#150E2B 0%,#1E1540 40%,#2A1F55 100%)':'linear-gradient(135deg,#4C1D95 0%,#6D28D9 40%,#8B5CF6 100%)' }}/>
          {[...Array(20)].map((_,i)=>(
      ...
      <main className="relative max-w-5xl mx-auto px-6 py-10">
        <div className={`p-6 rounded-2xl border mb-12 ${panelStyle}`}>
          <div className="flex justify-between items-center mb-3">
            <div>
              <p className={`text-[10px] uppercase tracking-widest font-black mb-0.5 ${dimText}`}>✦ Your Progress</p>
          ...
          <div className={`h-3 rounded-full overflow-hidden ${isDark?'bg-[#2A1F55]':'bg-[#F3EEF9]'}`}>
        ...
        <div className="space-y-0 relative">
          <div className={`hidden md:block absolute left-1/2 -translate-x-1/2 top-7 bottom-7 w-0.5 ${isDark?'bg-[#2A1F55]':'bg-[#E9DEFF]'}`}/>
          {roadmapSteps.map((step, i) => (
            <RoadmapStep key={step.id} step={step} index={i} darkMode={isDark}
