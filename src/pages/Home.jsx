...
const Home = ({ darkMode, setDarkMode }) => {
  ...
  const [showIntro, setShowIntro] = useState(() => { const flag = sessionStorage.getItem('showBookIntro'); if (flag === 'true') { sessionStorage.removeItem('showBookIntro'); return true; } const visited = sessionStorage.getItem('hasVisited'); if (!visited) { sessionStorage.setItem('hasVisited', 'true'); return true; } return false; });
  ...
  useEffect(() => { setIsLoggedIn(!!(localStorage.getItem('user')||localStorage.getItem('token'))); },[]);
  useEffect(() => { const fn=()=>setScrolled(window.scrollY>40); window.addEventListener('scroll',fn); return()=>window.removeEventListener('scroll',fn); },[]);
  useEffect(() => {
    const s=new IntersectionObserver(([e])=>{if(e.isIntersecting&&!hasAnimated){startCounter();setHasAnimated(true);}},{threshold:.2});
    const c=new IntersectionObserver(([e])=>{if(e.isIntersecting)setShowComm(true);},{threshold:.2});
    if(statsRef.current)s.observe(statsRef.current);
    if(commRef.current)c.observe(commRef.current);
    return()=>{s.disconnect();c.disconnect();};
  },[hasAnimated]);
  ...
  const handleSearch=e=>{e.preventDefault();if(searchQuery.trim())navigate(`/search?q=${encodeURIComponent(searchQuery)}`);};
  const handleLogout=()=>{localStorage.removeItem('token');localStorage.removeItem('user');setIsLoggedIn(false);setSidebarOpen(false);navigate('/');};
  const handleRandom=()=>{const r=dummyBooks[Math.floor(Math.random()*dummyBooks.length)];setSidebarOpen(false);navigate(`/book/${r.id}`,{state:{book:r}});};
  ...
  /* color shorthands */
  const dm=darkMode;
  const bg=dm?'bg-[#0F0A20]':'bg-[#FAF7FF]';
  const bgAlt=dm?'bg-[#150E2B]':'bg-[#F3EEF9]';
  const text=dm?'text-[#F3EEF8]':'text-[#2E1065]';
  const muted=dm?'text-[#C4B5FD]':'text-[#7C3AED]';
  const cardBg=dm?'bg-[#1E1540] border-[#2A1F55]':'bg-white border-[#E9DEFF]';
  const inp=`w-full px-4 pr-10 py-2 rounded-xl border outline-none text-sm transition-all focus:ring-2 ${dm?'bg-[#1E1540] border-[#2A1F55] text-[#F3EEF8] focus:border-[#8B5CF6] focus:ring-[#8B5CF6]/20 placeholder-[#6D28D9]':'bg-white/90 border-[#E9DEFF] text-[#2E1065] focus:border-[#8B5CF6] focus:ring-[#8B5CF6]/10 placeholder-[#A78BFA]'}`;

  if(showIntro) return <BookIntro onDone={()=>setShowIntro(false)}/>;

  return (
    <div className={`w-full min-h-screen ${bg} ${text} transition-colors duration-300 relative`}>
      <Petals/>

      {/* ══ NAVBAR ══ */}
      <nav className={`fixed top-0 w-full z-50 transition-all duration-400 ${
        scrolled
          ?(dm?'bg-[#0F0A20]/96 backdrop-blur-xl border-b border-[#2A1F55] shadow-[0_4px_30px_rgba(109,40,217,.18)]':'bg-white/96 backdrop-blur-xl border-b border-[#E9DEFF] shadow-lg')
          :'bg-transparent'
      }`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-16 gap-3">
            ...
            <Link to="/" onClick={() => sessionStorage.setItem('showBookIntro', 'true')} className="flex-shrink-0 group">
              <div className={`flex items-center gap-2 px-3 py-1.5 rounded-xl transition-all group-hover:scale-105 ${dm?'hover:bg-[#1E1540]':'hover:bg-[#F3EEF9]'}`}>
            ...
            <div className="hidden md:flex items-center gap-1">
              {[{l:'Browse',to:'/search'},{l:'Communities',to:'/groups'},{l:'Games',to:'/all-games'},...(isLoggedIn?[{l:'Profile',to:'/profile'}]:[])].map(i=>(
                <Link key={i.l} to={i.to} className={`nav-link px-3 py-2 rounded-lg text-sm font-medium transition-all ${dm?'text-[#C4B5FD] hover:text-white hover:bg-[#1E1540]':'text-[#6D28D9] hover:text-[#4C1D95] hover:bg-[#F3EEF9]'}`}>{i.l}</Link>
              ))}
            </div>
            ...
            <form onSubmit={handleSearch} className="flex-1 max-w-sm mx-2">
              <div className="relative">
                <input type="text" value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} placeholder="Search books, authors..." className={inp}/>
            ...
            <div className="flex items-center gap-2">
              {setDarkMode&&(
                <button onClick={()=>setDarkMode(!dm)} className={`w-9 h-9 rounded-xl flex items-center justify-center hover:scale-110 transition-all ${dm?'bg-[#1E1540] text-yellow-300':'bg-[#F3EEF9] text-[#6D28D9]'}`}>
                  {dm?'☀️':'🌙'}
                </button>
              )}
              {!isLoggedIn&&<><Link to="/login" className={`hidden sm:block text-sm font-medium nav-link ${dm?'text-[#C4B5FD]':'text-[#6D28D9]'}`}>Login</Link>
              ...
              <button onClick={()=>setSidebarOpen(true)} className={`group w-10 h-10 flex flex-col items-center justify-center rounded-xl gap-1.5 transition-all ${dm?'bg-[#1E1540] hover:bg-[#2A1F55]':'bg-[#F3EEF9] hover:bg-[#E9DEFF]'}`}>
      ...
      <div className={`fixed inset-0 z-[60] ${sidebarOpen?'pointer-events-auto':'pointer-events-none'}`}>
        ...
        <aside className={`absolute right-0 top-0 h-full w-[22rem] max-w-[92vw] overflow-y-auto transition-transform duration-300 ease-in-out ${sidebarOpen?'translate-x-0':'translate-x-full'} ${dm?'bg-[#0F0A20] border-l border-[#2A1F55]':'bg-[#FAF7FF] border-l border-[#E9DEFF]'}`}
          style={{ boxShadow:'-16px 0 50px rgba(109,40,217,.3)' }}>

          <div className={`sticky top-0 z-10 px-6 py-5 border-b backdrop-blur-xl ${dm?'bg-[#0F0A20]/95 border-[#2A1F55]':'bg-[#FAF7FF]/95 border-[#E9DEFF]'}`}>
            <div className="flex items-center justify-between">
              <Link to="/" onClick={()=>setSidebarOpen(false)} className="flex items-center gap-3 group">
                ...
                <div>
                  <span style={{ fontFamily:"'Playfair Display',serif" }} className={`font-black text-xl italic ${dm?'text-[#F3EEF8]':'text-[#4C1D95]'}`}>BiblioTech</span>
                  <p className={`text-[9px] tracking-[.22em] uppercase ${dm?'text-[#A78BFA]':'text-[#8B5CF6]'}`}>✦ Magical Library ✦</p>
                </div>
              </Link>
              <button onClick={()=>setSidebarOpen(false)} className={`w-9 h-9 rounded-xl flex items-center justify-center hover:rotate-90 transition-all duration-300 ${dm?'bg-[#1E1540] text-[#C4B5FD]':'bg-[#F3EEF9] text-[#6D28D9]'}`}>✕</button>
            </div>
          </div>

          <div className="px-6 py-6 space-y-7">
            {/* Auth */}
            <div className={`rounded-2xl p-5 border ${dm?'bg-gradient-to-br from-[#1E1540] to-[#0F0A20] border-[#2A1F55]':'bg-gradient-to-br from-[#F3EEF9] to-white border-[#E9DEFF] shadow-md'}`}>
              {isLoggedIn?(
                <><p className={`text-[10px] uppercase tracking-[.2em] font-bold mb-1 ${dm?'text-[#A78BFA]':'text-[#8B5CF6]'}`}>✦ Your Space</p>
                <h3 className="font-bold text-lg mb-3">Welcome back! ✨</h3>
                <div className="grid grid-cols-2 gap-2">
                  <Link to="/profile" onClick={()=>setSidebarOpen(false)}><button className={`w-full py-2.5 rounded-xl text-sm font-semibold hover:-translate-y-0.5 transition-all ${dm?'bg-[#2A1F55] text-white':'bg-[#F3EEF9] text-[#4C1D95]'}`}>My Profile</button></Link>
                ...
                <><p className={`text-[10px] uppercase tracking-[.2em] font-bold mb-1 ${dm?'text-[#A78BFA]':'text-[#8B5CF6]'}`}>✦ Begin Your Journey</p>
                <h3 className="font-bold text-lg mb-1">Join the Library ✨</h3>
                <p className={`text-xs mb-4 ${dm?'text-[#C4B5FD]':'text-[#6D28D9]'}`}>Unlock your full reading experience.</p>
                <div className="grid grid-cols-2 gap-2">
                  <Link to="/login" onClick={()=>setSidebarOpen(false)}><button className={`w-full py-2.5 rounded-xl text-sm font-semibold border hover:-translate-y-0.5 transition-all ${dm?'bg-[#1E1540] text-white border-[#2A1F55]':'bg-white text-[#4C1D95] border-[#E9DEFF]'}`}>Login</button></Link>
            ...
            {/* Browse */}
            <div>
              <p className={`text-[10px] uppercase tracking-[.3em] font-bold mb-3 ${dm?'text-[#A78BFA]':'text-[#8B5CF6]'}`}>✦ Browse</p>
              {[{l:'Best Sellers',to:'/search?type=collection&q=best-sellers',s:'★'},{l:'New Releases',to:'/search?type=collection&q=new-releases',s:'✦'},{l:'Popular Books',to:'/search?type=collection&q=popular-books',s:'♥'},{l:'Game Library',to:'/all-games',s:'✧'},{l:'Romance',to:'/search?type=genre&q=romance',s:'✿'},{l:'Academic',to:'/search?type=genre&q=academics',s:'◆'},{l:'Advanced Search',to:'/advanced',s:'⊕'}].map(i=>(
                <Link key={i.l} to={i.to} onClick={()=>setSidebarOpen(false)} className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all hover:-translate-x-1 ${dm?'hover:bg-[#1E1540] text-[#C4B5FD]':'hover:bg-[#F3EEF9] text-[#4C1D95]'}`}>
              ...
              <button onClick={handleRandom} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all hover:-translate-x-1 ${dm?'hover:bg-[#1E1540] text-[#C4B5FD]':'hover:bg-[#F3EEF9] text-[#4C1D95]'}`}>
            ...
            {/* Characters */}
            <div className={`rounded-2xl p-5 text-center border ${dm?'bg-[#1E1540] border-[#2A1F55]':'bg-[#F3EEF9] border-[#E9DEFF]'}`}>
              ...
              <p className={`text-xs italic mt-3 ${dm?'text-[#C4B5FD]':'text-[#6D28D9]'}`} style={{ fontFamily:"'Lora',serif" }}>"Every book is a new adventure!"</p>
            </div>
          </div>
        </aside>
      </div>
      ...
      <section className="relative h-screen w-full overflow-hidden">
        {/* BACKGROUND — soft gradient, no messy image overlay */}
        ...
        <div className="hidden lg:block absolute bottom-20 left-8" style={{ animation:'float 5s ease-in-out infinite' }}>
          ...
          <p className={`text-center text-xs font-bold italic mt-1 ${dm?'text-[#C4B5FD]':'text-[#8B5CF6]'}`} style={{ fontFamily:"'Lora',serif" }}>Owly</p>
        </div>
        <div className="hidden lg:block absolute bottom-16 right-6" style={{ animation:'float 5s 2s ease-in-out infinite' }}>
          ...
          <p className={`text-center text-xs font-bold italic mt-1 ${dm?'text-[#F9A8D4]':'text-[#EC4899]'}`} style={{ fontFamily:"'Lora',serif" }}>Wormy</p>
        </div>

        {/* HERO TEXT */}
        <div className="relative h-full flex flex-col items-center justify-center text-center px-4 pt-16">
          <div className="animate-fade-in max-w-4xl">
            <div className={`inline-flex items-center gap-2 px-5 py-2 rounded-full border mb-6 text-sm backdrop-blur-sm ${dm?'border-[#A78BFA]/30 bg-[#8B5CF6]/10 text-[#C4B5FD]':'border-[#A78BFA]/40 bg-[#8B5CF6]/8 text-[#6D28D9]'}`}>
            ...
            <h1 style={{ fontFamily:"'Playfair Display',serif" }}
              className={`text-5xl md:text-7xl lg:text-8xl font-black mb-6 leading-[.95] tracking-tight ${dm?'text-white':'text-[#2E1065]'}`}>
              Discover your<br/>
              <span className="text-shimmer">next obsession,</span><br/>
              <span className={`italic font-normal text-[.8em] ${dm?'text-[#C4B5FD]':'text-[#6D28D9]'}`}>one page at a time</span>
            </h1>

            <p className={`text-lg md:text-xl mb-10 max-w-2xl mx-auto leading-relaxed ${dm?'text-[#C4B5FD]':'text-[#6D28D9]'}`} style={{ fontFamily:"'Lora',serif" }}>
              A curated world of books, reader communities, and magical discoveries — all waiting for you.
            </p>

            <div className="flex flex-wrap items-center justify-center gap-4">
              <Link to="/search" className={`group flex items-center gap-2 px-8 py-4 rounded-2xl font-bold text-base shadow-xl transition-all hover:-translate-y-1 active:scale-95 ${dm?'bg-white text-[#4C1D95]':'bg-[#4C1D95] text-white'}`}>
                <span>Explore Books</span><span className="group-hover:translate-x-1 transition-transform">✦</span>
              </Link>
              <Link to="/register" className={`px-8 py-4 rounded-2xl font-bold text-base border backdrop-blur-sm transition-all hover:-translate-y-1 active:scale-95 ${dm?'border-[#A78BFA]/40 bg-[#8B5CF6]/20 text-[#C4B5FD]':'border-[#A78BFA]/50 bg-[#8B5CF6]/10 text-[#6D28D9]'}`}>
                Join Free ✨
              </Link>
            </div>
          </div>
          <div className={`absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 animate-fade-in ${dm?'text-[#A78BFA]/50':'text-[#8B5CF6]/50'}`} style={{ animationDelay:'1.5s' }}>
      ...
      {/* ══ STATS ══ */}
      <section ref={statsRef} className={`py-20 px-6 ${bgAlt}`}>
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-12">
            <p className={`text-xs uppercase tracking-[.3em] font-bold mb-2 ${muted}`}>✦ By the numbers</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <div key={i} className={`relative overflow-hidden p-8 rounded-3xl text-center border transition-all hover:-translate-y-2 ${cardBg}`}
                style={{ boxShadow:dm?'0 0 30px rgba(109,40,217,.08)':'0 8px 30px rgba(109,40,217,.07)' }}>
                ...
                <div className={`text-sm font-semibold uppercase tracking-widest mt-2 ${muted}`}>{s.l}</div>
      ...
      <section className={`py-16 px-6 ${bg}`}>
        <div className="max-w-5xl mx-auto text-center mb-10">
          <p className={`text-xs uppercase tracking-[.3em] font-bold mb-2 ${muted}`}>✦ Meet your companions</p>
          <h2 style={{ fontFamily:"'Playfair Display',serif" }} className="text-3xl font-bold">Your Library Guides</h2>
        </div>
        <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className={`group p-8 rounded-3xl border transition-all hover:-translate-y-2 ${cardBg}`}
            style={{ boxShadow:dm?'0 0 40px rgba(109,40,217,.08)':'' }}>
            <div className="flex items-center gap-5">
              ...
              <div>
                ...
                <p className={`text-sm leading-relaxed mb-3 ${dm?'text-[#C4B5FD]':'text-[#6D28D9]'}`}>Your wise library guardian. Owly's two different-coloured eyes see every story — one violet, one rose. He guards the shelves by night and picks your next great read.</p>
                <div className="flex flex-wrap gap-2">
                  {['Fantasy','Mystery','Classics'].map(t=><span key={t} className={`text-xs px-2.5 py-1 rounded-full font-semibold ${dm?'bg-[#2A1F55] text-[#C4B5FD]':'bg-[#F3EEF9] text-[#6D28D9]'}`}>{t}</span>)}
                </div>
              </div>
            </div>
          </div>
          <div className={`group p-8 rounded-3xl border transition-all hover:-translate-y-2 ${cardBg}`}
            style={{ boxShadow:dm?'0 0 40px rgba(219,39,119,.08)':'' }}>
            <div className="flex items-center gap-5">
              ...
              <div>
                ...
                <p className={`text-sm leading-relaxed mb-3 ${dm?'text-[#C4B5FD]':'text-[#6D28D9]'}`}>The spectacled bookworm who carries a full stack wherever he wriggles! Wormy has devoured every genre and will always find the perfect recommendation for you.</p>
                <div className="flex flex-wrap gap-2">
                  {['Romance','Sci-Fi','Self-help'].map(t=><span key={t} className={`text-xs px-2.5 py-1 rounded-full font-semibold ${dm?'bg-[#2A1F55] text-[#F9A8D4]':'bg-[#FFF0F7] text-[#DB2777]'}`}>{t}</span>)}
                </div>
              </div>
            </div>
          </div>
      ...
      {/* ══ COMMUNITY ══ */}
      <section ref={commRef} className={`py-16 px-6 ${bgAlt}`}>
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className={`p-10 rounded-3xl border transition-all duration-1000 ${showComm?'translate-x-0 opacity-100':'opacity-0 -translate-x-12'} ${cardBg}`}>
            <p className={`text-xs uppercase tracking-[.25em] font-bold mb-4 ${muted}`}>✦ Community</p>
            <h2 style={{ fontFamily:"'Playfair Display',serif" }} className="text-4xl font-bold mb-4 text-shimmer">Reader Communities</h2>
            <p className={`text-base leading-relaxed mb-6 ${dm?'text-[#C4B5FD]':'text-[#6D28D9]'}`}>Don't read alone. Join our global network of book lovers. <strong>Create your own community</strong>, host <strong>Discussion Boards</strong>, and connect with readers who share your passion.</p>
            <div className="flex flex-wrap gap-2">
              {['#DiscussionBoard','#CreateCommunity','#BookClub','#ReadingGoals'].map(t=>(
                <span key={t} className={`px-3 py-1.5 rounded-full text-xs font-bold border ${dm?'bg-[#2A1F55]/50 border-[#2A1F55] text-[#C4B5FD]':'bg-[#F3EEF9] border-[#E9DEFF] text-[#6D28D9]'}`}>{t}</span>
              ))}
            </div>
          </div>
          <div className={`relative min-h-[300px] rounded-3xl overflow-hidden transition-all duration-1000 delay-300 ${showComm?'translate-x-0 opacity-100':'translate-x-12 opacity-0'}`}
            style={{ background:dm?'linear-gradient(135deg,#1E1540,#2A1F55)':'linear-gradient(135deg,#4C1D95,#7C3AED,#DB2777)' }}>
            <Stars/>
      ...
      <section className={`py-20 px-6 ${bgAlt}`}>
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-12">
            <p className={`text-xs uppercase tracking-[.3em] font-bold mb-2 ${muted}`}>✦ Find your next read</p>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
            {categories.map(c=>(
              <Link key={c.name} to={`/search?type=genre&q=${c.name.toLowerCase()}`}
                className={`category-pill group flex flex-col items-center gap-2 p-4 rounded-2xl border text-center transition-all ${dm?'bg-[#1E1540] border-[#2A1F55] hover:bg-[#2A1F55] hover:border-[#A78BFA]':'bg-white border-[#E9DEFF] hover:bg-gradient-to-br hover:from-[#6D28D9] hover:to-[#DB2777] hover:border-transparent hover:text-white shadow-sm'}`}>
      ...
      <section className={`py-20 px-6 ${bg}`}>
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-12">
            <p className={`text-xs uppercase tracking-[.3em] font-bold mb-2 ${muted}`}>✦ Reach us</p>
            <h2 style={{ fontFamily:"'Playfair Display',serif" }} className="text-4xl font-bold">Send a message</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
            <div>
              <p className={`text-base leading-relaxed mb-8 ${dm?'text-[#C4B5FD]':'text-[#6D28D9]'}`}>Feel free to reach out with questions, feedback, or suggestions. We reply faster than Wormy can finish a chapter!</p>
              <div className={`flex items-center gap-4 p-4 rounded-2xl border mb-8 ${cardBg}`}>
                ...
                <div>
                  <p className={`text-[10px] uppercase tracking-widest font-bold mb-0.5 ${muted}`}>Email</p>
              ...
              <div className="flex items-end gap-4">
                ...
                <div className={`px-4 py-3 rounded-2xl rounded-bl-none text-sm italic max-w-xs border ${cardBg}`} style={{ fontFamily:"'Lora',serif" }}>
                  <span className={dm?'text-[#C4B5FD]':'text-[#6D28D9]'}>"We reply faster than Wormy can finish a chapter! ✨"</span>
                </div>
              </div>
            </div>
            <div className={`p-8 rounded-3xl border ${cardBg}`}>
              <label className={`block text-[10px] uppercase tracking-widest font-bold mb-1.5 ${muted}`}>Your message</label>
              <textarea rows="5" placeholder="Write your message here..."
                className={`w-full p-4 rounded-2xl border outline-none resize-none transition-all focus:ring-2 mb-4 ${dm?'bg-[#0F0A20] border-[#2A1F55] text-[#F3EEF8] focus:border-[#8B5CF6] focus:ring-[#8B5CF6]/20 placeholder-[#4C1D95]':'bg-[#F3EEF9] border-[#E9DEFF] text-[#2E1065] focus:border-[#8B5CF6] focus:ring-[#8B5CF6]/10 placeholder-[#8B5CF6]'}`}/>
